...
function draw()
{
    ...
    var volscale = (vol === 0) ? 0 : 0.01/vol;
	
	// Scale segment x length distribution using bass energy levels.
	segrangex[0] = volscale * (2 + energyl / 1);
	segrangex[1] = volscale * (5 + energyl / 0.5);

	// Scale segment y length distribution using mid energy levels and
	// high-mid energy levels.
	segrangey[0] = volscale * (-energym);
	segrangey[1] = volscale * energym;
	...
	fill(fillvar, 1, 1, 0.03);
    stroke(1-fillvar, 1, 1, 0.06);
